...
public class ProblemSolutions {
    ...
    public static void selectionSort(int[] values, boolean ascending ) {
        ...
        for (int i = 0; i < n - 1; i++) {
            ...
            for (int j = i + 1; j < n; j++) {
                if (ascending ? values[j] < values[extremeIndex] : values[j] > values[extremeIndex]) {
                    extremeIndex = j;
                }
            }
    ...
    private void mergeDivisbleByKFirst(int arr[], int k, int left, int mid, int right)
    {
        ...
        while (i <= mid && j <= right) {
            boolean leftDivisible = arr[i] % k == 0;
            boolean rightDivisible = arr[j] % k == 0;

            if (leftDivisible && !rightDivisible || (arr[i] <= arr[j] && leftDivisible == rightDivisible)) {
                temp[idx++] = arr[i++];
            } else {
                temp[idx++] = arr[j++];
            }
